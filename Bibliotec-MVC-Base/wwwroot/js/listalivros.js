...
// Lógica de Reserva com SweetAlert2
const botoesReservar = document.querySelectorAll('.btn_reservar');
botoesReservar.forEach(btn => {
    btn.addEventListener('click', () => {
        const id = btn.getAttribute('data-id');
        const titulo = btn.getAttribute('data-titulo');
        ...
        Swal.fire({
            ...
            text: `Deseja realmente reservar o livro "${titulo}"?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                fetch(`/Reserva/Reservar?LivroId=${id}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    }
                    ...
                    if (response.ok) {
                        Swal.fire(
                            'Reservado!',
                            'Sua reserva foi criada com sucesso. Fique atento ao prazo de devolução!',
                            'success'
                        );
                    } else {
                        const data = await response.json().catch(() => null);
                        const msg = data && data.mensagem ? data.mensagem : 'Ocorreu um problema ao tentar reservar o livro.';
                        Swal.fire(
                            'Atenção!',
                            msg,
                            'warning'
